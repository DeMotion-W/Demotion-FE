...
export default function DemoListView({ demoList, token }: { demoList: Demo[]; token: string }) {
  const [sortOrder, setSortOrder] = useState("최신순");
  const [currentPage, setCurrentPage] = useState(1);
  const [openMenuId, setOpenMenuId] = useState<number | null>(null);

  const totalPages = Math.ceil(demoList.length / ITEMS_PER_PAGE);

  const sortedList = useMemo(() => {
    return [...demoList].sort((a, b) => {
      if (sortOrder === "최신순") {
  ...
  const currentItems = sortedList.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
    ...
    <>
      <div className="flex justify-between items-center h-[60px] mb-2">
        <div className="flex gap-2 items-center justify-center text-sm text-[#6B7280] font-medium font-['Pretendard'] leading-normal">
          <p>총 항목</p>
          <p className="text-[#3E82F6]">{demoList.length}</p>
        </div>
        ...
        <div className="w-full grid grid-cols-4 gap-6">
          {currentItems.map((demo, i) => (
            <DemoCard
              ...
              onToggleMenu={() => setOpenMenuId((prev) => (prev === demo.demoId ? null : demo.demoId))}
            />
          ))}
        </div>
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`w-8 h-8 text-xs cursor-pointer ${
                currentPage === i + 1 ? "text-[#191F28]" : "text-[#8B95A1] hover:bg-gray-100"
              }`}
            >
              {i + 1}
            </button>
